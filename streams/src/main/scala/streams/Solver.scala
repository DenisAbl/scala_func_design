...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      case im if im.isEmpty => LazyList.empty
      case (block, moves) #:: tail =>  {
        val updExplored =  explored + block
        val moreMoves = newNeighborsOnly(neighborsWithHistory(block, moves), updExplored)
        moreMoves ++ from(tail ++ moreMoves, updExplored)
      }
    }
  }
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from((startBlock,Nil) #:: LazyList.empty, Set())
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case paths if paths.isEmpty => Nil
    case moves => moves.head._2.reverse
  }
}
