...
trait DecoderInstances {
  ...
  implicit def intDecoder: Decoder[Int] = {
    Decoder.fromFunction{case Json.Num(x) if x.isValidInt => Option(x.toInt)
                         case _ => None
    }
  ...
  implicit def stringDecoder: Decoder[String] = {
    Decoder.fromFunction{case Json.Str(value) => Option(value)}
  }
  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit def boolDecoder: Decoder[Boolean] = {
    Decoder.fromFunction{case Json.Bool(value) => Option(value)}
  }
  /**
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(list) => {
        val array = list.map(obj => decoder.decode(obj))
        array.contains(None) match {
          case true => None
          case false => Option(array.flatten)
        }
      }
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      .zip(Decoder.field[Int]("age")).transform[Person](cons => Person(cons._1, cons._2))
